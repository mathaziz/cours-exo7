...
\begin{enumerate}
	...
	Comme $B$ n'est pas fermé, $C$ n'est pas ouvert.
	...
	Ce n'est pas non plus un fermé : en effet soit $x_n = (\frac1n,0)$, la suite $(x_n)_{n\ge1}$ est une suite d'éléments de $H$ et tend vers $(0,0) \notin H$. Ainsi par la caractérisation d'un fermé par les suites, $H$ n'est pas fermé.
	
	L'adhérence de $H$ est $\bar H = H \cup (\{0\} \times [0,1])$, c'est-à-dire qu'on rajoute une dent au-dessus de l'origine. L'intérieur de $H$ est vide.
...
\begin{enumerate}
	...
	Pour chaque $k$, choisissons $x_k\in C_k$. Comme $C_k \subset C_0$ alors $(x_k)_{k\in\Nn}$ est une suite d'éléments de $C_0$. Comme $C_0$ est compact alors $(x_k)_{k\in\Nn}$ admet une sous-suite  $(x_{\phi(k)})_{k\in\Nn}$ convergente. Notons $x_{\infty}$ la limite de cette sous-suite.
